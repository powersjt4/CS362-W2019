...
public class UrlValidator implements Serializable {
    ...
    //BUG FIX private static final String PATH_REGEX = "^(/[-\\w:@&?=+,.!*'%$_;\\(\\)]*)?$";
    ...
    public UrlValidator(String[] schemes, RegexValidator authorityValidator, long options) {
        ...
        if (isOn(ALLOW_ALL_SCHEMES)) {
            ...
            for(int i=0; i < schemes.length; i++) {
               //BUF FIX allowedSchemes.add(schemes[i].toUpperCase(Locale.ENGLISH));
 ...
 // BUG FIX      if ("http".equals(scheme)) {// Special case - file: allows an empty authority
